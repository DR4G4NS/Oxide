...
cargo build --release
mkdir -p "$class_dir"
javac -d "$class_dir" -cp "$desktop_jar" tools/smoke/SmokeUnitClear158.java
run_dir="$(mktemp -d "$project_dir/target/smoke-unit-clear-158.XXXXXX")"
...
sleep 0.5
java ${SMOKE_BUILD:+-Doxide.smoke.build=$SMOKE_BUILD} -cp "$desktop_jar:$class_dir" SmokeUnitClear158 "$smoke_port"

if ! grep -q "finished loading the world" "$server_log" \
        || ! grep -q '"unit_id": 2500000' "$save_file"; then
    echo "Server did not persist the replacement core unit. Artifacts: $run_dir" >&2
    exit 1
fi

echo "Server confirmed exact UnitClear lifecycle. Artifacts: $run_dir"
