...
/** Verifies ForceBuild sync after authoritative projectile absorption on desktop 158.1. */
public final class SmokeForce158 {
    private static final int FORCE = (50 << 16) | 100;
    ...
    public static void main(String[] args) throws Exception {
        int port = args.length == 0 ? 6588 : Integer.parseInt(args[0]);
        Version.build = 158;
        Vars.headless = true;
        ...
        Vars.world.tile(50, 100).setBlock(Blocks.forceProjector, Team.sharded, 0);
        ...
        AtomicBoolean verifiedState = new AtomicBoolean();
        ...
        client.addListener(new NetListener() {
            ...
            public void connected(Connection connection) {
                ...
                packet.mods = new Seq<>();
                packet.name = "force-158";
                packet.locale = "en";
                packet.uuid = "Zm9yY2UxNTg=";
                packet.usid = "";
            ...
            public synchronized void received(Connection connection, Object object) {
                try {
                    if (object instanceof Packets.StreamBegin begin) {
                        ...
                        packet.handled();
                        if (packet.amount == 1 && verifyForce(packet)
                                && verifiedState.compareAndSet(false, true)) {
                            verified.countDown();
                        }
                    }
            ...
            public void disconnected(Connection connection, DcReason reason) {
                if (!verifiedState.get()) {
                    System.err.println("disconnected before Force verification: " + reason);
                    verified.countDown();
                }
            }
        });
        ...
        try {
            ...
            if (!verified.await(5, TimeUnit.SECONDS) || !verifiedState.get()) {
                throw new AssertionError("ForceProjector snapshot was not verified");
            }
            Thread.sleep(800);
            if (!client.isConnected()) throw new AssertionError("client disconnected");
            System.out.println(
                "ok forceProjector=true officialReadSync=true"
                    + " shieldBuildup=true power=true");
        } finally {
            client.stop();
        }
    }

    private static boolean verifyForce(BlockSnapshotCallPacket packet) throws Exception {
        ByteArrayInputStream bytes = new ByteArrayInputStream(packet.data);
        DataInputStream input = new DataInputStream(bytes);
        int position = input.readInt();
        int block = input.readShort();
        if (position != FORCE || block != Blocks.forceProjector.id) return false;
        ForceBuild build = (ForceBuild)Vars.world.tile(50, 100).build;
        build.readSync(Reads.get(input), build.version());
        if (build.buildup <= 0f || build.power == null || build.power.status < 0.99f
                || build.warmup < 0.9f || build.radscl <= 0f
                || build.phaseHeat != 0f || bytes.available() != 0) {
            throw new AssertionError(
                "ForceBuild mismatch: broken=" + build.broken
                    + " buildup=" + build.buildup + " radscl=" + build.radscl
                    + " warmup=" + build.warmup + " phaseHeat=" + build.phaseHeat
