 ...
 */
public final class SmokeJoin1597 {
    private static void validateStreamedRules(byte[] compressed) {
        try (var input = new java.io.DataInputStream(new java.util.zip.InflaterInputStream(
            ...
            mindustry.game.Rules rules = mindustry.io.JsonIO.read(
                mindustry.game.Rules.class, input.readUTF());
            if (rules.loadout == null) throw new AssertionError("null loadout");
            System.out.println("SMOKE_OK typed-rules loadout=" + rules.loadout);
        } catch (Exception error) {
            throw new AssertionError("client cannot deserialize streamed Rules", error);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        AtomicBoolean beginBreakSeen = new AtomicBoolean();
        AtomicBoolean deconstructFinishSeen = new AtomicBoolean();
            ...
            public synchronized void received(Connection connection, Object object) {
                if (object instanceof Packets.StreamBegin begin) {
                    ...
                    if (world.size() == streamTotal.get()
                        ...
                        validateStreamedRules(world.toByteArray());
                    ...
                    packet.handled();
                    if (packet.x != 45 || packet.y != 100 || packet.result.id != 216) return;
                    constructionResponses.countDown();
                } else if (object instanceof ConstructFinishCallPacket packet) {
                    packet.handled();
                    if (packet.tile == null || packet.tile.x != 45 || packet.tile.y != 100 || packet.block.id != 216) return;
                    constructionResponses.countDown();
            ...
            if (!deconstructionResponses.await(5, TimeUnit.SECONDS)) {
                throw new AssertionError(
                    "missing deconstruction responses=" + deconstructionResponses.getCount()
                        + " beginBreak=" + beginBreakSeen.get()
                        + " deconstructFinish=" + deconstructFinishSeen.get()
                        + " protocolError=" + client.getLastProtocolError());
            }
